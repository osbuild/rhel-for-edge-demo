...
## Alternative Install using the more advanced kickstart Example

The second kickstart example, [`edge2.ks`](edge2.ks), will install the same ostree commit and also demonstrate the use of a few features in `%post` that makes the OS much easier to manage when using a large fleet of systems. To use this example simply adjust the kickstart being used:
```
...
Containers are an ideal vehicle to help ensure that "patch Tuesday" problems do not cause  disruptions. The boinc example container shows a modified unit file generated by `podman generate systemd`. The container will be managed via systemd to ensure it's running at startup and in the case that it fails at runtime, automatically restarted via `Restart=on-failure`.
...
## Final Thoughts
When nodes are deployed in numbers ranging from 10s of thousands into the millions, we need to think differently about how we interact with the fleet. As connectivity inceases with Edge devices, the importance of keeping systems and containers/applications properly patched and updated while also minimizing downtime for updates and application disruptions. The example kickstarts demonstrate how to configure nodes to automatically stay currect on available updates. These can be built upon and adapted for other use cases. 
